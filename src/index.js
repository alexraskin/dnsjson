...
function returnHeader(title = 'Turning dig... to JSON') {
	html = `<!DOCTYPE html>
          ...
          <title>dnsjson.00z.sh | ${title}</title>
          ...
          <a class="navbar-brand" href="/">dnsjson.00z.sh</a>
          </div>
          </div>
          </nav>`;
	return html;
...
function returnFooter() {
	html = `<footer class="footer">
          <div class="container">
          <p class="text-muted">&copy; <script type="text/javascript">document.write(new Date().getFullYear());</script> dnsjson.00z.sh - This script is written using only <a href="https://cloudflareworkers.com" target="_blank">Cloudflare Workers</a></p>',
          </div>
          </footer>
...
async function fetchAndRespond(request) {
	...
	const supportedTypes = ['A', 'AAAA', 'CAA', 'CNAME', 'DS', 'DNSKEY', 'MX', 'NS', 'NSEC', 'NSEC3', 'RRSIG', 'SOA', 'TXT'];
	...
	if (reqUrl == '/') {
		...
		if (request.method == 'GET') {
          ...
          If you need this, then dnsjson.00z.sh is for you!
          </p>
          <p>&nbsp;</p>
          ...
          `;
			output = [returnHeader('Turning dig... to JSON'), data, returnFooter()];
...
<pre>curl https://dnsjson.00z.sh/${res['results']['name']}/${res['results']['type']}.json</pre>

          <p>
          Need the results in plain text?
          </p>
<pre>curl https://dnsjson.00z.sh/${res['results']['name']}/${res['results']['type']}.txt</pre>
          </div>
          </div>
          <div class="panel panel-primary">
          <div class="panel-heading">
          ...
          <button type="submit" class="btn btn-primary">Lookup</button>
			...
			output = [returnHeader(res['results']['name'] + ' (' + res['results']['type'] + ')'), data, returnFooter()];
			return new Response(output.join('\r\n'), {
				headers: {
					'Content-Type': 'text/html',
					'Access-Control-Allow-Origin': '*',
